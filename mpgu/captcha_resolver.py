...
class Resolver:
    ...
    def _get_image(self, session: Session) -> bytes:
        ...
        print(f"get token in image: {csrf_token=}")

        _csrf = self.session.cookies.get('_csrf')
...
def login() -> LoginDetails:
    ...
    print(f'\n-----------------\nCaptcha resolver DEBUG: {captcha_value=}')

    _csrf = resolver.session.cookies.get('_csrf')
    phpsessid = resolver.session.cookies.get('PHPSESSID')
    cookie = f'PHPSESSID={phpsessid}; _csrf={_csrf}'

    print(f'Captcha resolver DEBUG: {csrf_token=}')
    print(f'Captcha resolver DEBUG: {cookie=}')
    ...
    phpsessid = resolver.session.cookies.get('PHPSESSID')
    cookie = f'PHPSESSID={phpsessid}; _csrf={_csrf}'
    print(f'Captcha resolver DEBUG: after {cookie=}')

    soup = BeautifulSoup(r.text, 'html.parser')
    csrf_token_div = soup.find("meta", {"name": "csrf-token"})
    csrf_token = csrf_token_div['content']
    print(f'Captcha resolver DEBUG: after {csrf_token=}')
